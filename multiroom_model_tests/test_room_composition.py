import unittest
from multiroom_model.roomcomposition import RoomComposition  # Change this to match your file/module name
...
class TestRoomComposition(unittest.TestCase):
    ...
    def test_correct_sum_explicit_other(self):
        rc = RoomComposition(soft=10, paint=10, wood=10, metal=10, concrete=10,
    ...
    def test_raises_if_sum_above_100(self):
        with self.assertRaises(ValueError):
            RoomComposition(soft=50, paint=30, wood=30, other=10)  # Total = 120

    def test_raises_if_sum_below_100_and_other_given(self):
        with self.assertRaises(ValueError):
            RoomComposition(soft=20, paint=20, wood=20, other=10)  # Total = 70

    def test_invalid_material_values(self):
        with self.assertRaises(ValueError):
            RoomComposition(soft=-10, paint=10, wood=10, other=90)  # Negative soft

        with self.assertRaises(ValueError):
            RoomComposition(soft=101)  # Soft > 100

    def test_surface_area_dictionary_scaling(self):
        rc = RoomComposition(soft=50, wood=50)
